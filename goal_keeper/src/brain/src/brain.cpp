...
void Brain::tick()
{
    ...
    tree->tick();
     //高添加
         // 获取 gc_game_state 标志的值
        std::string gc_game_state = tree->getEntry<std::string>("gc_game_state");
        std::string decision = tree->getEntry<std::string>("decision");
        if ((tickCounter % 900 == 0&&data->ballDetected&&decision != "kick")||gc_game_state == "READY") {
        tree->setEntry<bool>("locate_and_correct", true);
        }//优化了抬头策略
    tickCounter++;

    if(last_SeeMode!=tree->getEntry<bool>("locate_and_correct")&&tree->getEntry<bool>("locate_and_correct")==false)
    {
         SeeTime = tickCounter;//加上判断条件，防止因为抬头而丢失自身的球坐标
    }    
    if(last_detectballstate!=data->ballDetected&& data->ballDetected==false)
    {
         Nodetectballtime = tickCounter;//球是否获取去噪
    } 
    if(last_decision!=tree->getEntry<std::string>("decision")&& tree->getEntry<std::string>("decision")=="kick")
    {
        lastkicktime = tickCounter;//kick time
    }  
    last_SeeMode = tree->getEntry<bool>("locate_and_correct");
    last_detectballstate = data->ballDetected;
    last_decision = tree->getEntry<std::string>("decision");
}
...
void Brain::updateBallMemory()
{
    ...
    if (get_clock()->now().seconds() - data->ball.timePoint.seconds() > config->memoryLength )//config->memoryLength
    {
        tree->setEntry<bool>("ball_location_known", false);
        data->ballDetected = false;
    }
        //定位过10s刷新找球//8.6新增
    if (get_clock()->now().seconds() - data->ball.timePoint.seconds() > 7.5 )//config->memoryLength
    {
        tree->setEntry<bool>("ball_location_known", true);
        data->ball.timePoint = get_clock()->now();
    }
    //定位组添加old
    //bool teammate_see_sign = //tree->getEntry<bool>("teammate_see_sign");
    bool locate_and_correct = tree->getEntry<bool>("locate_and_correct");
    //teammate_see_sign = true    ; ////just test  如果不在这里强制赋值为true，下面就不满足teammate_see_sign的if判断条件
    //new:
     // 1. 取队友球信息（线程安全）
    auto teammateBallMap = getTeammateBallInfoMap();
    auto now = get_clock()->now();
    const double MAX_AGE = 1.0;          // 信息有效期 1 秒，可按需改
    ...
    // 2. 遍历找最近且有效的球
    for (const auto& [id, info] : teammateBallMap) {
        if (!info.see_sign) continue;                         // 没看到球
        double age = (now - info.lastUpdate).seconds();
        if (age > MAX_AGE) continue;                          // 太旧
        if (info.ball_dist < bestDist) {                      // 更近
           bestDist = info.ball_dist;
           bestId   = id;
           bestX    = info.ball_x;
           bestY    = info.ball_y;
        }
    ...
    if (bestId != -1) {
        fused_ball_x = bestX;
        fused_ball_y = bestY;
        //data->ball.range        = bestDist;   // 也可以保留原值
        //data->ballDetected      = true;
        //tree->setEntry<bool>("ball_location_known", true);
        }

    
    if (!data->ballDetected && bestId != -1 && !locate_and_correct && (tickCounter-SeeTime > 150)&& (tickCounter-lastkicktime> 250)&&(tickCounter-Nodetectballtime > 50) )  //8.5gengxin去噪，8.6kicktime
    {
            data->ball.posToField.x = fused_ball_x;//tree->getEntry<float>("fused_ball_x");
            data->ball.posToField.y = fused_ball_y;//tree->getEntry<float>("fused_ball_y");
            // data->ball.posToField.x = 0;
            // data->ball.posToField.y = 0;
        }
...
NodeStatus LocateAndCorrectPosition::tick()
{
    bool locate_and_correct = brain->tree->getEntry<bool>("locate_and_correct");
//    if(locate_and_correct)
//    {
 //   brain->client->moveHead(-0.2, 0.0);  // 抬头
//    rclcpp::Rate(1).sleep();            // 等待 1 秒
//    brain->tree->setEntry<bool>("odom_calibrated", false); // 强制重定位
//    rclcpp::Rate(0.5).sleep();          // 再留 0.5 秒给视觉
//    //brain->client->moveHead(0.0, 0.0);  // 恢复默认
//    brain->tree->setEntry<bool>("locate_and_correct", false);
 //   }
...
std::shared_mutex _teammate_ball_info_mutex;  // 声明一个读写锁

std::map<int, BrainCommunication::TeammateBallInfo> Brain::getTeammateBallInfoMap() {
    std::shared_lock<std::shared_mutex> lock(_teammate_ball_info_mutex);  // 获取共享锁，允许并发读取
    return communication->_teammate_ball_info_map;
}
