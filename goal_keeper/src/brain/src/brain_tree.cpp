...
void BrainTree::initEntry()
{
    ...
    setEntry<double>("ball_y_pos", 0);
    setEntry<double>("angeltoball", 0);
...
NodeStatus GoalieDecide::tick()
{
    ...
    brain->tree->setEntry<double>("ball_y_pos", bally);
    brain->tree->setEntry<double>("angeltoball", dir_rb_f);
    ...
    // 只有当球超过-3.0米，或y大于3或小于-3时，才继续retreat
    else if (brain->data->ball.posToField.x > -4.0 || brain->data->ball.posToField.y > 3.0 || brain->data->ball.posToField.y < -3.0)
    {
        newDecision = "retreat";
        color = 0xFF00FFFF;
    }
    ...
    else if (brain->data->ballDetected && angleIsGood)
    {
        newDecision = "kick";
        color = 0xFF0000FF;
    }
