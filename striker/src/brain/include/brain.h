...
public:
    ...
    std::map<int, BrainCommunication::TeammateBallInfo> getTeammateBallInfoMap();
    float fused_ball_x = 0.0;
    float fused_ball_y = 0.0;
...
private:
    ...
    int tickCounter = 0;   //加上
    int SeeTime = 0;      //加上
    bool last_SeeMode = false ;  //加上
    int Nodetectballtime = 0;      //加上
    bool last_detectballstate = false ;  //加上
    int lastkicktime = 0;  //plus
    std::string last_decision = ""; //加上

};

class LocateAndCorrectPosition : public SyncActionNode
{
public:
    LocateAndCorrectPosition(const std::string& name, const NodeConfig& config, Brain* _brain)
