...
public:
    ...
    {
        return {
            InputPort<double>("chase_threshold", 0.8, "Perform the chasing action if the distance exceeds this threshold"),
...
public:
    ...
    static PortsList providedPorts()
    {
        return {
            ...
            InputPort<double>("dist", 0.6, "The target distance behind the ball for chasing it"),
...
// 传球节点：PassBall
class PassBall : public SyncActionNode
{
public:
    PassBall(const std::string &name, const NodeConfig &config, Brain *_brain) : SyncActionNode(name, config), brain(_brain) {}

    static PortsList providedPorts()
    {
        return {
            InputPort<double>("min_pass_dist", 1.0, "最小传球距离，距离小于该值不传球"),
            InputPort<double>("max_pass_dist", 5.0, "最大传球距离，距离大于该值不传球"),
            InputPort<double>("pass_angle_tolerance", 0.3, "允许的传球角度误差（弧度）"),
            InputPort<int>("target_id", -1, "指定传球目标队友ID，-1为自动选择"),
            OutputPort<double>("pass_target_x"),
            OutputPort<double>("pass_target_y"),
            OutputPort<double>("pass_target_theta"),
            OutputPort<int>("target_id")
        };
    }
...
private:
    ...
    // 新增辅助函数
    void calculateAndSetPassTarget(const GameObject &ball,
