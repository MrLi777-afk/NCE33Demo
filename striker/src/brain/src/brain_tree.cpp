...
void BrainTree::init()
{
    ...
    REGISTER_BUILDER(PrintMsg)
        ...
        //添加!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    //REGISTER_BUILDER(CheckLocateAndCorrect)
...
void BrainTree::initEntry()
{
    ...
    setEntry<double>("ball_direction_on_field", 0.0);
...
NodeStatus GoalieDecide::tick()
{
    ...
    // 只有当球超过-3.0米，才继续retreat
    else if (brain->data->ball.posToField.x > -3.0)
    {
        newDecision = "retreat";
        color = 0xFF00FFFF;
    }
...
NodeStatus PassBall::tick()
{
    ...
    if (!brain->tree->getEntry<bool>("ball_location_known")) {
        brain->log->logToScreen("tree/PassBall", format("can not detect ball,can not pass ball"), 0xFF0000FF);
        return NodeStatus::FAILURE;
    }
    ...
    if (teammate_addresses.empty()) {
        brain->log->logToScreen("tree/PassBall", format("can not detect teammate,can not pass ball"), 0xFF0000FF);
        return NodeStatus::FAILURE;
    }
    ...
    if (target_id > 0) {
            ...
            if (info.playerId == target_id) {
                ...
                if (info_age > MAX_INFO_AGE_SEC) {
                    brain->log->logToScreen("tree/PassBall", 
                        format("set teammate ID:%d not update(%.2f s)", target_id, info_age), 0xFFFF00FF);
                    continue;
                }
                ...
                if (dist < min_pass_dist || dist > max_pass_dist) {
                    brain->log->logToScreen("tree/PassBall", 
                        format("set teammate ID:%d distance can not passball(%.2fm)", target_id, dist), 0xFFFF00FF);
                    break;
                }
        ...
        // 如果指定的队友不满足条件，记录日志
        brain->log->logToScreen("tree/PassBall", 
            format("set teammate ID:%d can not pass ball,try to find other teammate", target_id), 0xFFFF00FF);
    }
    ...
    if (!found_valid_teammate) {
        brain->log->logToScreen("tree/PassBall", format("can not find valid teammate,can not pass ball"), 0xFF0000FF);
        return NodeStatus::FAILURE;
    }
    // 5. 计算传球点并输出
                                        ...
                                        int teammate_id)
{
    ...
    // 记录传球目标信息
    brain->log->logToScreen("tree/PassBall", 
        format("pass ball to teammate ID:%d, target point:(%.2f,%.2f), angle:%.2f", 
        teammate_id, target_x, target_y, target_theta), 0x00FF00FF);
}
