...
void BrainTree::init()
{
    ...
    REGISTER_BUILDER(PassBall)

    // Action Nodes for debug
    REGISTER_BUILDER(PrintMsg)
    
    REGISTER_BUILDER(LocateAndCorrectPosition)   //声明
...
void BrainTree::initEntry()
{
    ...
    setEntry<double>("ball_y_pos", 0.0);
    setEntry<double>("ball_x_pos", 0.0);
...
//     // ---------- 新增 / 修改部分开始 ----------
//     constexpr double Y_EQUAL_TH = 0.10;      
//     static bool yAlignedOnce = false;  
...
//     double orientErr = toPInPI(kickDir - brain->data->robotPoseToField.theta);

//     // 判断 y 是否相等（在阈值内）
//     double yDiff = std::abs(brain->data->robotPoseToField.y - brain->data->ball.posToField.y);
//     if (!yAlignedOnce && yDiff < Y_EQUAL_TH)
//         yAlignedOnce = true;

//     // 根据是否已经对齐过选择 headingErr
//     double headingErr = yAlignedOnce ? orientErr : ballYaw;
//     // ---------- 新增 / 修改部分结束 ----------
...
//     { // chase
//         _state = "chase";
//         target_f.x = brain->data->ball.posToField.x - dist * cos(kickDir);
//         target_f.y = brain->data->ball.posToField.y - dist * sin(kickDir);
//     }
...
//     double vx = target_r.x;
//     double vy = target_r.y;
//     double vtheta = headingErr * 2.0;

//     double linearFactor = 1 / (1 + exp(3 * (ballRange * fabs(headingErr)) - 3));
//     vx *= linearFactor;
//     vy *= linearFactor;
...
NodeStatus Chase::tick()
{
    ...
    double orientErr = toPInPI(kickDir - brain->data->robotPoseToField.theta);
    double headingErr = (ballRange > R_switch) ? ballYaw     // 远：对着球
                                               : orientErr;  // 近：对准射门方向
    Pose2D target_f, target_r;
    if (brain->data->robotPoseToField.x - brain->data->ball.posToField.x > (_state == "chase" ? 1.0 : 0.0))
    {
...
NodeStatus SimpleChase::tick()
{
    double stopDist, stopAngle, vxLimit, vyLimit;
    getInput("stop_dist", stopDist);
    getInput("stop_angle", stopAngle);
    getInput("vx_limit", vxLimit);
    getInput("vy_limit", vyLimit);
    ...
    // 球相对机器人（机器人坐标系）
    const double r   = brain->data->ball.range;         // 与球的距离
    const double phi = brain->data->ball.yawToRobot;    // 球相对机器人朝向角（机器人->球）

    // 径向误差（正：向球前进；负：离球后退）
    const double dr = r - stopDist;

    // 控制参数
    const double kLin   = 0.8;   // 线速度比例
    const double kTheta = 2.0;   // 角速度比例
    const double distTol = 0.05; // 距离容差，可视需要调整
    const double vthetaCap = 1.0; // 角速度上限（未作为端口，固定值防抖）

    double vx = 0.0, vy = 0.0, vtheta = 0.0;

    // 角度控制：始终朝向足球
    if (std::abs(phi) > stopAngle)
        vtheta = cap(kTheta * phi, vthetaCap, -vthetaCap);
    else
        vtheta = 0.0;

    // 线速度控制：保持在 stopDist 距离
    if (std::abs(dr) > distTol)
    {
        // 沿着机器人->球方向（phi）移动，收敛到目标半径
        const double v = kLin * dr;       // 速度标量（带正负号）
        vx = v * std::cos(phi);
        vy = v * std::sin(phi);
...
NodeStatus AssistAdjust::tick()
{
    ...
    double kickDir;
    if (position == "defense") {
        kickDir = atan2(brain->data->ball.posToField.y,
                        brain->data->ball.posToField.x + brain->config->fieldDimensions.length / 2 - 1.0);
    } else { 
        kickDir = (brain->data->ball.posToField.y > 0) ?  80.0 * M_PI / 180.0 : -80.0 * M_PI / 180.0;
    }
    double dir_rb_f = brain->data->robotBallAngleToField;
    double deltaDir = toPInPI(kickDir - dir_rb_f);
    double dir = deltaDir > 0 ? -1.0 : 1.0;
...
NodeStatus StrikerDecide::tick()
{
    ...
    double kickDir = (position == "defense") ? atan2(brain->data->ball.posToField.y, brain->data->ball.posToField.x + brain->config->fieldDimensions.length / 2) : atan2(-brain->data->ball.posToField.y, brain->config->fieldDimensions.length / 2 - brain->data->ball.posToField.x);
    double dir_rb_f = brain->data->robotBallAngleToField;
    double gp_margin = brain->data->ball.posToField.x < 0 ? -1.0 : 0.8;   // 根据球在场地前后切换
    auto goalPostAngles = brain->getGoalPostAngles(gp_margin);
    double theta_l = goalPostAngles[0];
    double theta_r = goalPostAngles[1];
    bool angleIsGood;
    string gc_game_sub_state_type = brain->tree->getEntry<string>("gc_game_sub_state_type");
    bool gc_is_sub_state_kickoff_side = brain->tree->getEntry<bool>("gc_is_sub_state_kickoff_side");
    
    if (gc_game_sub_state_type == "FREE_KICK" && gc_is_sub_state_kickoff_side) {
        // 任意球状态下的特殊角度判断
        if (brain->data->ball.posToField.y > 0) {
            // 球在上半场，只有dir_rb_f为-80度时angleIsGood才为true
            double target_angle = 80.0 * M_PI / 180.0;  // -80度转弧度
            angleIsGood = (fabs(dir_rb_f - target_angle) < 0.1);  // 允许0.1弧度的误差
        } else {
            // 球在下半场，只有dir_rb_f为80度时angleIsGood才为true
            double target_angle = -80.0 * M_PI / 180.0;   // 80度转弧度
            angleIsGood = (fabs(dir_rb_f - target_angle) < 0.1);  // 允许0.1弧度的误差
        }
    } else {
        // 非任意球状态，使用原来的规则
        angleIsGood = (theta_l > dir_rb_f && theta_r < dir_rb_f);
    }
    double ballRange = brain->data->ball.range;
    double ballYaw = brain->data->ball.yawToRobot;
    ...
    if (brain->tree->getEntry<bool>("ball_location_known"))
    {
       brain->tree->setEntry<double>("ball_direction_on_field", brain->data->robotBallAngleToField);
    }
    string newDecision;
    ...
    // 优先判断射门/踢球条件，避免被chase抢先
    else if (brain->data->ballDetected && angleIsGood && brain->data->ball.posToField.x > 3.5 && ballRange <= chaseRangeThreshold)
    {
        newDecision = "shot";  // 在射门区域且角度好，大力射门
        color = 0xFF0000FF;
    }
    else if (brain->data->ballDetected && angleIsGood && ballRange <= chaseRangeThreshold)
    {
        newDecision = "kick";  // 角度好但不在射门区域，小力踢球
        color = 0xFF0000FF;
    }
    else if (ballRange > chaseRangeThreshold * (lastDecision == "chase" ? 0.9 : 1.0))
    {
        newDecision = "chase";  // 距离太远，先chase
        color = 0x00FF00FF;
    }
    else
    {
        newDecision = "adjust";  // 距离合适但角度不好，调整角度
        color = 0x00FFFFFF;
    }
...
NodeStatus PassBall::tick()
{
    double min_pass_dist, max_pass_dist, pass_angle_tolerance;
    int target_id;
    getInput("min_pass_dist", min_pass_dist);
    getInput("max_pass_dist", max_pass_dist);
    getInput("pass_angle_tolerance", pass_angle_tolerance);
    getInput("target_id", target_id);
    // 1. 获取自身和球的位置
    auto &self_pose = brain->data->robotPoseToField;
    auto &ball = brain->data->ball;
    if (!brain->tree->getEntry<bool>("ball_location_known")) {
        brain->log->logToScreen("tree/PassBall", "球未被检测到，无法传球", 0xFF0000FF);
        return NodeStatus::FAILURE;
    }

    auto &teammate_addresses = brain->communication->_teammate_addresses;
    // 检查是否有队友信息
    if (teammate_addresses.empty()) {
        brain->log->logToScreen("tree/PassBall", "未检测到队友，无法传球", 0xFF0000FF);
        return NodeStatus::FAILURE;
    }

    auto current_time = brain->get_clock()->now();
    const double MAX_INFO_AGE_SEC = 2.0; // 信息最大有效期为2秒

    if (target_id > 0) {
        for (const auto &pair : teammate_addresses) {
            const auto &info = pair.second;
            
            // 检查是否是指定的队友
            if (info.playerId == target_id) {
                // 检查信息是否过时
                double info_age = (current_time - info.lastUpdate).seconds();
                if (info_age > MAX_INFO_AGE_SEC) {
                    brain->log->logToScreen("tree/PassBall", 
                        "指定队友ID:" + std::to_string(target_id) + "的信息已过时(" + 
                        std::to_string(info_age) + "秒)", 0xFFFF00FF);
                    continue;
                }
                
                // 检查距离是否合适
                double dist = hypot(info.x - self_pose.x, info.y - self_pose.y);
                if (dist < min_pass_dist || dist > max_pass_dist) {
                    brain->log->logToScreen("tree/PassBall", 
                        "指定队友ID:" + std::to_string(target_id) + "距离不适合传球(" + 
                        std::to_string(dist) + "米)", 0xFFFF00FF);
                    break;
                }
                
                // 计算传球点并输出
                calculateAndSetPassTarget(ball, info, target_id);
                return NodeStatus::SUCCESS;
            }
        }
        ...
        // 如果指定的队友不满足条件，记录日志
        brain->log->logToScreen("tree/PassBall", 
            "指定队友ID:" + std::to_string(target_id) + "不满足传球条件，尝试寻找其他队友", 0xFFFF00FF);
    }
    // 2. 遍历所有队友，找出距离自己最近且大于2米的队友
    int best_id = -1;
    double min_dist = 1e9;
    BrainCommunication::TeammateInfo best_mate{};
    bool found_valid_teammate = false;
    for (const auto &pair : teammate_addresses) {
        const auto &info = pair.second;
        double info_age = (current_time - info.lastUpdate).seconds();
        if (info_age > MAX_INFO_AGE_SEC) {
        ...
        if (dist < min_dist) {
            min_dist = dist;
            best_id = info.playerId;
            best_mate = info;
            found_valid_teammate = true;
        }
    }

    if (!found_valid_teammate) {
        brain->log->logToScreen("tree/PassBall", "未找到合适的传球目标", 0xFF0000FF);
        return NodeStatus::FAILURE;
    }
    // 5. 计算传球点并输出
    calculateAndSetPassTarget(ball, best_mate, best_id);
    return NodeStatus::SUCCESS;
}
void PassBall::calculateAndSetPassTarget(const GameObject &ball, 
                                        const BrainCommunication::TeammateInfo &mate, 
                                        int teammate_id)
{
    // 计算球到队友的方向向量
    double dx = mate.x - ball.posToField.x;
    double dy = mate.y - ball.posToField.y;
    double norm = std::hypot(dx, dy);
    
    // 计算球后方的目标点（距离球0.5米）
    double offset = 0.5; 
    double target_x = ball.posToField.x - offset * dx / norm;
    double target_y = ball.posToField.y - offset * dy / norm;
    double target_theta = std::atan2(dy, dx);
    
    // 输出目标参数到端口
    setOutput("pass_target_x", target_x);
    setOutput("pass_target_y", target_y);
    setOutput("pass_target_theta", target_theta);
    setOutput("target_id", teammate_id);
    
    // 记录传球目标信息
    brain->log->logToScreen("tree/PassBall", 
        "传球目标ID:" + std::to_string(teammate_id) + 
        ", 目标点:(" + std::to_string(target_x) + "," + 
        std::to_string(target_y) + "), 角度:" + 
        std::to_string(target_theta), 0x00FF00FF);
}
