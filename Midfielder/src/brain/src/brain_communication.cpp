...
void BrainCommunication::broadcastDiscovery() {
    while (_broadcast_discovery_flag)
    {
        ...
        msg.robot_x = brain->data->robotPoseToField.x;
        msg.robot_y = brain->data->robotPoseToField.y;
        msg.robot_theta = brain->data->robotPoseToField.theta;
        msg.ball_x = brain->data->ball.posToField.x;
        msg.ball_y = brain->data->ball.posToField.y;
        msg.ball_dist = brain->data->ball.range;
        msg.See_sign = brain->data->ballDetected;
...
void BrainCommunication::spinDiscoveryReceiver() {    
    ...
    while (_receive_discovery_flag) {
        ...
        if (msg.validation != VALIDATION_DISCOVERY) continue; // fail to pass validation
        ...
        if (msg.playerId == brain->config->playerId) {  // ignore self messages
            ...
            std::lock_guard<std::mutex> ball_lock(_teammate_ball_info_mutex);
                _teammate_ball_info_map[msg.playerId] = {
                    msg.playerId,
                    msg.ball_x,
                    msg.ball_y,
                    msg.ball_dist,  
            ...
            // // 定位新增
            // last_teammate_ball_x = curr_teammate_ball_x;
            // last_teammate_ball_y = curr_teammate_ball_y;
            // teammate_see_sign    = true;//msg.See_sign;fortest=true
            // if(teammate_see_sign)
            // {
            //     curr_teammate_ball_x = 2.0;//msg.ball_x;for test=0
            //     curr_teammate_ball_y = 2.0;//msg.ball_y; 
            // }
            //     // === 追加：计算队友共享足球均值 ===
            // brain->teammate_see_sign1 = teammate_see_sign;
            // if (teammate_see_sign)
            // {
            //     brain->fused_ball_x = (last_teammate_ball_x + curr_teammate_ball_x) * 0.5;
            //     brain->fused_ball_y = (last_teammate_ball_y + curr_teammate_ball_y) * 0.5;
            //     //brain->tree->setEntry<float>("fused_ball_x", fused_x);
            //     //brain->tree->setEntry<float>("fused_ball_y", fused_y);
            // }
